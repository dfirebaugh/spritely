...
void register_draw_fn(void (*f)(void))
{
  js_draw = (*f);
}

void register_update_fn(void (*f)(void))
{
  js_update = (*f);
}

void render_rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t color)
{
  SDL_Rect new_rect;
  ...
  SDL_SetRenderDrawColor(renderer, color_values[color], color_values[color+1], color_values[color+2], 255);
  SDL_RenderDrawRect(renderer, &new_rect);
}

void render_rect_fill(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t color)
{
  SDL_Rect new_rect;
