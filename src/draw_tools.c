...
static void udpate_sprite_sheet(void) {
  Context_swap_pixels(sprite_selector_cells[current_sprite_index],
                      sprite_canvas_ctx);
}
...
static void clear_pixels_to_fill(void) {
...
/**
 * tool_fill_recurse
 * recursive function that fills in neighbors with the current `pen_color` if
 * the conditions are correct
 */
static void tool_fill_recurse(const unsigned int rect_index,
...
void left_clicks(void) {
  ...
  Context_handle_rect_click(color_picker_ctx, tool_color_pick, mouse.x,
                            mouse.y);
  Context_handle_rect_click(sprite_selector_ctx, tool_sprite_selection, mouse.x,
                            mouse.y);
  Context_handle_rect_click(toolbar_ctx, tool_toolbar_selection, mouse.x,
                            mouse.y);
}

void left_drags(void) {
  if (active_tool == DRAG) {
    Context_handle_rect_click(sprite_canvas_ctx, tool_drag, mouse.x, mouse.y);
...
void increment_sprite_selector(void) {
...
void decrement_sprite_selector(void) {
  if (!(sprite_sheet_index_in_range(current_sprite_index - 1)))
    return;
...
void increment_row_sprite_selector(void) {
  if (!(sprite_sheet_index_in_range(current_sprite_index +
                                    SPRITESHEET_ROW_SIZE)))
    return;
...
void decrement_row_sprite_selector(void) {
  if (!(sprite_sheet_index_in_range(current_sprite_index -
                                    SPRITESHEET_ROW_SIZE)))
    return;
...
void draw_tool_activate_pen(void) {
  active_tool = PEN;
  Message_Queue_enqueue(command_message_queue, "pen tool", 0);
  Context_indicator_focus(toolbar_ctx, PEN);
}

void draw_tool_activate_fill(void) {
  active_tool = FILL;
...
void draw_tool_handle_undo(void) {
  undo();
  Context_indicator_focus(toolbar_ctx, PEN);
}

void draw_tool_handle_redo(void) {
  redo();
  Context_indicator_focus(toolbar_ctx, PEN);
}

void draw_tool_handle_open_file(void) {
  open_file();
