...
static void populate_file_data_with_spritesheet_row(uint8_t *const file_data,
                                                    const int row) {
  const int sprite_start = row * SPRITESHEET_ROW_SIZE;
  int file_data_index =
      row * SPRITESHEET_ROW_SIZE * SPRITE_CANVAS_SIZE * NUM_COLOR_COMPONENTS;
  ...
  for (int r = 0; r < SPRITE_CANVAS_ROW_SIZE; r += 1) {
    ...
    for (int s = sprite_start; s < sprite_start + SPRITESHEET_ROW_SIZE;
         s += 1) {
      Context_t sprite = sprite_selector_cells[s];

      // Iterate pixels in the sprite canvas row
      for (int p = pixel_start; p < pixel_start + SPRITE_CANVAS_ROW_SIZE;
           p += 1) {
...
static void save_spritesheet(const char *const filename) {
  uint8_t
      file_data[NUM_COLOR_COMPONENTS * SPRITESHEET_SIZE * SPRITE_CANVAS_SIZE];
  ...
  const int write_code = stbi_write_png(
      ...
      NUM_COLOR_COMPONENTS, file_data,
      SPRITE_CANVAS_ROW_SIZE * SPRITESHEET_ROW_SIZE *
          NUM_COLOR_COMPONENTS // stride in bytes
  );

  if (write_code == 0) {
    Message_Queue_enqueue(command_message_queue, "Failed to save spritesheet",
                          0);
    return;
  }
...
static void save_sprites(
  ...
  for (int y = 0; y < SPRITESHEET_COL_SIZE; y += 1) {
    for (int x = 0; x < SPRITESHEET_ROW_SIZE; x += 1) {
      ...
      if (write_code == 0) {
        Message_Queue_enqueue(command_message_queue, "Error outputting sprites",
                              0);
...
void save_file(int save_individual_sprites) {
  ...
  sfd_Options opt = {.filter_name = "Image File", .filter = "*.png"};
  ...
  if (save_individual_sprites) {
    const size_t filename_prefix_len =
        find_last_occurrence(filename, strlen(filename), '.');
...
static void
populate_spritesheet_row_with_file_data(const uint8_t *const file_data,
                                        const int row) {
  const int sprite_start = row * SPRITESHEET_ROW_SIZE;
  int file_data_index =
      row * SPRITESHEET_ROW_SIZE * SPRITE_CANVAS_SIZE * NUM_COLOR_COMPONENTS;
...
void open_file(void) {
  ...
  sfd_Options opt = {.filter_name = "Image File",
                     .filter = "*.png",
  ...
  // We are guaranteed to get NUM_COLOR_COMPONENTS number of channels in the
  // result if successful
  const unsigned char *const image_data =
      stbi_load(filename, &image_width, &image_height, &num_channels,
                NUM_COLOR_COMPONENTS);

  if (image_data == NULL) {
    // "The function stbi_failure_reason() can be queried for an extremely
    // brief, end-user
    //  unfriendly explanation of why the load failed." via stb_image.h
    Message_Queue_enqueue(command_message_queue,
  ...
  if (image_width != desired_width || image_height != desired_height) {
    Message_Queue_enqueue(command_message_queue,
                          "Given spritesheet has incorrect dimensions", 0);
